...
This script is different from the InitialStep script as it also requires a folder of already taken 
...
step1Dir = "/path to your new save folder/"
step0Dir = "/path to folder with images to align to/"
Alignment = "/location to save small images taken for alignment purposes/"
...
def find_small_image_in_large(large_image_path, small_image_path):
    Large_Image = io.imread(large_image_path)
    Small_Image = io.imread(small_image_path)

    result = match_template(Large_Image, Small_Image)
    ij = np.unravel_index(np.argmax(result), result.shape)
    x, y = ij[::-1]

    y2 = y + Small_Image.shape[0]
    x2 = x + Small_Image.shape[1]
    Small_Image_from_large_image = Large_Image[y:y2, x:x2]

    subset_center_x = x + Small_Image.shape[1] / 2
    subset_center_y = y + Small_Image.shape[0] / 2

    large_image_center_x = Large_Image.shape[1] / 2
    large_image_center_y = Large_Image.shape[0] / 2

    vector_x = large_image_center_x - subset_center_x
    vector_y = large_image_center_y - subset_center_y
...
if not os.path.exists(step0Dir):
    ...
    print("Step0Dir did not exist. Please upload images here:")
...
#opens a windows explorer for the save directory.
subprocess.Popen(['explorer', step1Dir])
...
movement_calculation_time = 60 #seconds per image to template match 
estimated_total_time = total_images*(dic_dwell * (HR_resolution_width * HR_resolution_height) + focus_routine_time + movement_calculation_time)
...
root.geometry("1500x400")  # Adjust the window size for the additional text

# Create a label for the title
title_label = tk.Label(root, text="Experiment in Progress: Do not disturb.", font=("Helvetica", 16))
title_label.pack(pady=10)

# Create labels to display the entered name and email
name_label = tk.Label(root, text=f"If needed, contact {Name} at {email} for help.", font=("Helvetica", 12))
name_label.pack()
...
time_label = tk.Label(root, text="Time Remaining: Calculating...", font=("Helvetica", 12))
time_label.pack()

# Store the start time
start_time = time.time()

# Calculate the expected end time based on the estimated total time
#expected_end_time = start_time + estimated_total_time
#expected_end_time_str = datetime.datetime.fromtimestamp(expected_end_time).strftime("%Y-%m-%d %H:%M:%S")


# Function to update the time label with hours and minutes
...
with tqdm(total=total_images, desc="Capturing Images") as pbar:
    for j in range(start_y_index, yDim): #y shifts
        for i in range(start_x_index, xDim): #xshifts
            ...
            # Update the progress bar, time, and expected end time in the GUI window
            ...
            ################ Code to Match Centers of Images ##########################
            # Step 1: Take Small image at specified zoom and resolution
            microscope.beams.electron_beam.horizontal_field_width.value = (1024 / 6144) * hfw  # Converts from 6144 hfw to an image with 768x512 pixels.
            imgSettings = GrabFrameSettings(resolution="1024x884", dwell_time=dic_dwell)  # Maybe change the dwell time --> test
            image = microscope.imaging.grab_frame(imgSettings)
            image.save("{}//{}_{}.tif".format(Alignment, j + 1, col + 1))

            # Step 2: Match Images to calculate the displacement vector
            large_image_path = os.path.join(step0Dir, f"{j + 1}_{col + 1}.tif")
            small_image_path = os.path.join(Alignment, f"{j + 1}_{col + 1}.tif")

            vector_x, vector_y = find_small_image_in_large(large_image_path, small_image_path)

            # Step 3: Convert the pixel displacement to real units?
            displacement_vector_x = (hfw / 6144) * vector_x
            displacement_vector_y = (hfw / 6144) * (-1*vector_y)

            # Step 4: Move the microscope stage
            # Calculate the new x and y positions of the stage
            new_x_position = microscope.specimen.stage.current_position.x + displacement_vector_x
            new_y_position = microscope.specimen.stage.current_position.y + displacement_vector_y

            #Update base location based on template matching results to ensure correct images for large xDim or yDim
            rawX = rawX + displacement_vector_x
            rawY = rawY + displacement_vector_y

            # Move the microscope stage to the new x and y positions
            microscope.beams.electron_beam.horizontal_field_width.value = hfw
            microscope.specimen.stage.absolute_move(StagePosition(x=new_x_position, y=new_y_position))
            time.sleep(stage_delay)  # Wait for the stage to settle

            print("Moved to new position (X, Y):", microscope.specimen.stage.current_position.x * 1e6, microscope.specimen.stage.current_position.y * 1e6)
            ...
            image = microscope.imaging.grab_frame(imgSettings)
            image.save("{}//{}_{}.tif".format(step1Dir, j+1, col+1))
